...
15b) Eval corpus expansion - STATUS: [~]
   - DoD: Corpus expanded beyond the initial seed into a broader set of snapshot cases (controls, dialogs, lists/nav, drift variants, basic forms, shortcuts), each tagged with a category so by-category metrics are meaningful; still short of the long-term 20-30+ target and without hard "lift threshold" gating, which remains future work. Retrieval/healer behavior is exercised both offline (deterministic harness) and online against Postgres/pgvector via integration tests that seed multiple embeddings and assert the correct selector is ranked above distractors. Planner QA intents (nav, scroll, download, URL asserts, submit, error text) are covered by a small regression corpus plus end-to-end QA live-run harness.
   - Tests: Summary aggregation unit test plus corpus invariants (size, categories) under "engine/tests/eval"; healer eval harness tests ("engine/tests/eval/test_healer_retrieval.py"); planner QA eval tests ("engine/tests/eval/test_planner_qa_corpus_eval.py", "test_e2e_qa_runs.py"); Postgres/pgvector integration tests for retrieval and privacy ("engine/tests/integration/test_retrieval_pg.py", "test_retrieval_privacy_pg.py", "test_retrieval_lift_pg.py"). Future integration runs will produce stable metrics once corpus and thresholds are finalized.
