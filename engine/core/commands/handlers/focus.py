...
class FocusHandler(IActionHandler):
    ...
    def execute(self, tool_call: dict, ctx: ExecCtx) -> StepResult:
        ...
        try:
            if callable(runner):
                runner(self._browser.focus(resolved))
            else:
                asyncio.run(self._browser.focus(resolved))
            return StepResult(ok=True)
        except Exception:
            return StepResult(ok=False, reason="focus_failed")
