...
class ClearHandler(IActionHandler):
    ...
    def execute(self, tool_call: dict, ctx: ExecCtx) -> StepResult:
        ...
        try:
            if callable(runner):
                runner(self._browser.clear(resolved))
            else:
                asyncio.run(self._browser.clear(resolved))
            return StepResult(ok=True)
        except Exception:
            return StepResult(ok=False, reason="clear_failed")
