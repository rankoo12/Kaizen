...
class SwitchTabHandler(IActionHandler):
    ...
    def execute(self, tool_call: dict, ctx: ExecCtx) -> StepResult:
        ...
        try:
            if callable(runner):
                runner(self._browser.switch_tab(index=index, url_contains=url_contains, title_contains=title_contains))
            else:
                asyncio.run(self._browser.switch_tab(index=index, url_contains=url_contains, title_contains=title_contains))
            return StepResult(ok=True)
        except Exception:
            return StepResult(ok=False, reason="switch_tab_failed")
