...
class CloseTabHandler(IActionHandler):
    ...
    def execute(self, tool_call: dict, ctx: ExecCtx) -> StepResult:
        ...
        try:
            if callable(runner):
                runner(self._browser.close_tab(index=index))
            else:
                asyncio.run(self._browser.close_tab(index=index))
            return StepResult(ok=True)
        except Exception:
            return StepResult(ok=False, reason="close_tab_failed")
