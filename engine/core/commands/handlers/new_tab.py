...
class NewTabHandler(IActionHandler):
    ...
    def execute(self, tool_call: dict, ctx: ExecCtx) -> StepResult:
        ...
        try:
            if callable(runner):
                runner(self._browser.new_tab(url))
            else:
                asyncio.run(self._browser.new_tab(url))
            return StepResult(ok=True)
        except Exception:
            return StepResult(ok=False, reason="new_tab_failed")
